...
public class hoMove : MonoBehaviour
{
    ...
    internal IEnumerator NextWaypoint()
    {
        ...
        for (int point = 0; point < wpPos.Length - 1; point++)
        {
            ...
            yield return StartCoroutine(SendMessages());
	...
	public void ChangeSpeed(float value)
    {
        ...
        if (tween == null)
        {
            Debug.LogWarning(gameObject.name + " has no tween to change its speed. "
                            + "Maybe it didn't initialize yet or you've stopped it before?");
            return;
        }
